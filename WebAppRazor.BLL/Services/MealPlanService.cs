...
namespace WebAppRazor.BLL.Services
{
    public class MealPlanService : IMealPlanService
    {
        ...
        public async Task<MealPlanDto> GenerateMenuAsync(int userId, double targetCalories, bool isPremium)
        {
            ...
            // Get user's health profile for AI context
            ...
            {
                // Use AI-generated menu
                plan.MealItems = aiResult.MealItems.Select(item => new MealItem
                {
                    ...
                    Ingredients = isPremium ? item.Ingredients : string.Empty,
                    CookingInstructions = isPremium ? item.CookingInstructions : string.Empty
                }).ToList();
            }
            else
            {
                // Fallback to hardcoded menu if AI fails
